...
Paydos Turizm`;
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        console.log('✅ Supabase\'den yüklendi');
      } catch (err) {
        console.error('❌ Supabase hatası:', err);
        // Boş başlat - default veri EKLEME
        setCustomers([]);
        setVisaApplications([]);
        setTours([]);
        setHotelReservations([]);
  ...
  const addCustomer = async (customer) => {
    ...
    const newCustomer = toCamelCase(data[0]);
    newCustomer.tags = newCustomer.tags || [];
    newCustomer.activities = newCustomer.activities || [];
  ...
  const updateCustomer = async (customer) => {
    const snakeData = toSnakeCase({ ...customer, updatedAt: new Date().toISOString() });
  ...
  const addVisa = async (visa) => {
    ...
    if (error) { console.error(error); return null; }
    const newVisa = toCamelCase(data[0]);
  ...
  const updateVisa = async (visa) => {
    const snakeData = toSnakeCase({ ...visa, updatedAt: new Date().toISOString() });
    const { error } = await supabase.from('visa_applications').update(snakeData).eq('id', visa.id);
    if (error) { console.error(error); return false; }
    setVisaApplications(prev => prev.map(v => v.id === visa.id ? visa : v));
    return true;
  };
  ...
  const addTour = async (tour) => {
    ...
    if (error) { console.error(error); return null; }
    const newTour = toCamelCase(data[0]);
  ...
  const updateTour = async (tour) => {
    const snakeData = toSnakeCase({ ...tour, updatedAt: new Date().toISOString() });
  ...
  const addHotel = async (hotel) => {
    ...
    if (error) { console.error(error); return null; }
    const newHotel = toCamelCase(data[0]);
    setHotelReservations(prev => [newHotel, ...prev]);
    return newHotel;
  };

  // Otel güncelle
  const updateHotel = async (hotel) => {
    const snakeData = toSnakeCase({ ...hotel, updatedAt: new Date().toISOString() });
    const { error } = await supabase.from('hotel_reservations').update(snakeData).eq('id', hotel.id);
    if (error) { console.error(error); return false; }
    setHotelReservations(prev => prev.map(h => h.id === hotel.id ? hotel : h));
    return true;
  };

  // Otel sil
  const deleteHotel = async (id) => {
    const { error } = await supabase.from('hotel_reservations').delete().eq('id', id);
    if (error) { console.error(error); return false; }
    setHotelReservations(prev => prev.filter(h => h.id !== id));
    return true;
  };
  ...
  useEffect(() => {
    if (!isLoading && islemFirmalari.length > 0) {
      saveSettings('firmalar', islemFirmalari);
    }
  }, [islemFirmalari, isLoading]);

  // Etiketler değişince kaydet
  useEffect(() => {
    if (!isLoading && musteriEtiketleri.length > 0) {
      saveSettings('etiketler', musteriEtiketleri);
    }
    ...
    <div style={{ display: 'flex', minHeight: '100vh', maxWidth: '100vw', overflow: 'hidden', background: 'linear-gradient(135deg, #0c1929 0%, #1a3a5c 50%, #0d2137 100%)', fontFamily: "'Segoe UI', sans-serif", color: '#e8f1f8' }}>
      ...
      <main style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, maxWidth: '100%', overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: isMobile ? '12px' : '16px', maxWidth: '100%' }}>
          ...
          {activeModule === 'customers' && <CustomerModule customers={customers} addCustomer={addCustomer} updateCustomer={updateCustomer} deleteCustomer={deleteCustomer} isMobile={isMobile} musteriEtiketleri={musteriEtiketleri} />}
          {activeModule === 'visa' && <VisaModule visaApplications={visaApplications} addVisa={addVisa} updateVisa={updateVisa} deleteVisa={deleteVisa} customers={customers} addCustomer={addCustomer} updateCustomer={updateCustomer} isMobile={isMobile} setActiveModule={setActiveModule} islemFirmalari={islemFirmalari} whatsappMesajlar={whatsappMesajlar} />}
          {activeModule === 'tours' && <TourModule tours={tours} addTour={addTour} updateTour={updateTour} deleteTour={deleteTour} customers={customers} isMobile={isMobile} />}
          {activeModule === 'hotels' && <HotelModule hotelReservations={hotelReservations} addHotel={addHotel} updateHotel={updateHotel} deleteHotel={deleteHotel} customers={customers} isMobile={isMobile} />}
...
function CustomerModule({ customers, addCustomer, updateCustomer, deleteCustomer, isMobile, musteriEtiketleri }) {
  ...
  const handleSave = async () => {
    ...
    if (editingCustomer) { 
      ...
      const updatedCustomer = { ...editingCustomer, ...formData, activities: updatedActivities };
      await updateCustomer(updatedCustomer);
      setEditingCustomer(updatedCustomer); 
      setFormData(prev => ({ ...prev, activities: updatedActivities }));
    }
    else { 
      const n = { ...formData, activities: [newActivity], createdAt: new Date().toISOString().split('T')[0] }; 
      const saved = await addCustomer(n); 
      if (saved) { setEditingCustomer(saved); setFormData(saved); }
    }
  ...
  const handleDelete = async (id) => { if (window.confirm('Silmek istediğinizden emin misiniz?')) await deleteCustomer(id); };
...
function VisaModule({ visaApplications, addVisa, updateVisa, deleteVisa, customers, addCustomer, updateCustomer, isMobile, setActiveModule, islemFirmalari, whatsappMesajlar }) {
  ...
  const handleSave = async () => {
    ...
    if (customer && activities.length > 0) {
      const updatedCustomer = { ...customer, activities: [...(customer.activities || []), ...activities] };
      await updateCustomer(updatedCustomer);
    }
    
    if (editingVisa) {
      await updateVisa(newVisa);
    } else {
...
function TourModule({ tours, addTour, updateTour, deleteTour, customers, isMobile }) {
  ...
  const emptyTour = {
    name: '', destination: '', startDate: '', endDate: '', duration: 1, price: '', currency: '€',
    capacity: 20, status: 'Planlama', description: '', includedServices: '', excludedServices: '',
    departureFlightNo: '', departureFlightTime: '', departureAirport: '', arrivalAirport: '',
    returnFlightNo: '', returnFlightTime: '', returnDepartureAirport: '', returnArrivalAirport: '',
    transferNotes: '', guideName: '', guidePhone: '', emergencyContact: '', notes: '',
    dailyProgram: [], participants: []
  };
  const [formData, setFormData] = useState(emptyTour);
  const [activeTab, setActiveTab] = useState('general');

  const statusOptions = ['Planlama', 'Satışta', 'Dolu', 'Devam Ediyor', 'Tamamlandı', 'İptal'];
  const statusColors = { 'Planlama': '#94a3b8', 'Satışta': '#3b82f6', 'Dolu': '#f59e0b', 'Devam Ediyor': '#8b5cf6', 'Tamamlandı': '#10b981', 'İptal': '#ef4444' };

  const filteredTours = tours.filter(t => {
    if (filterStatus !== 'all' && t.status !== filterStatus) return false;
    if (!searchTerm) return true;
    return t.name?.toLowerCase().includes(searchTerm.toLowerCase()) || t.destination?.toLowerCase().includes(searchTerm.toLowerCase());
  });
  ...
  const handleDateChange = (field, value) => {
    ...
    if (newData.startDate && newData.endDate) {
      ...
      // Generate daily program template
      if (newData.dailyProgram.length === 0 || window.confirm('Günlük program sıfırlansın mı?')) {
        const days = [];
        ...
        for (let i = 0; i < newData.duration; i++) {
          ...
          days.push({
            ...
            morningTime: '09:00', morningActivity: '',
            afternoonTime: '13:00', afternoonActivity: '',
            eveningTime: '19:00', eveningActivity: '',
            hotel: '', meals: { breakfast: false, lunch: false, dinner: false }, notes: ''
          });
        }
        newData.dailyProgram = days;
      }
    }
    setFormData(newData);
  };

  const updateDayProgram = (dayIndex, field, value) => {
    const updated = [...formData.dailyProgram];
    if (field.startsWith('meals.')) {
      const meal = field.split('.')[1];
      updated[dayIndex] = { ...updated[dayIndex], meals: { ...updated[dayIndex].meals, [meal]: value } };
    } else {
      updated[dayIndex] = { ...updated[dayIndex], [field]: value };
    }
    setFormData({ ...formData, dailyProgram: updated });
  };

  const handleSave = async () => {
    if (!formData.name) { alert('Tur adı zorunludur!'); return; }
    if (!formData.destination) { alert('Destinasyon zorunludur!'); return; }
    if (!formData.startDate || !formData.endDate) { alert('Tarihler zorunludur!'); return; }
    
    if (editingTour) {
      await updateTour({ ...editingTour, ...formData });
    } else {
      await addTour({ ...formData, createdAt: new Date().toISOString() });
    }
    setSaveMessage('Kaydedildi ✓');
    setTimeout(() => { setSaveMessage(''); setView('list'); setEditingTour(null); setFormData(emptyTour); }, 1500);
  };
  ...
  const addParticipant = (customer) => {
    if (formData.participants.find(p => p.customerId === customer.id)) {
      alert('Bu müşteri zaten ekli!'); return;
    }
    const participant = {
      customerId: customer.id,
    ...
    setFormData({ ...formData, participants: [...formData.participants, participant] });
  };

  const removeParticipant = (customerId) => {
    setFormData({ ...formData, participants: formData.participants.filter(p => p.customerId !== customerId) });
  };

  const updateParticipant = (customerId, field, value) => {
    setFormData({
      ...formData,
      participants: formData.participants.map(p => p.customerId === customerId ? { ...p, [field]: value } : p)
    });
  };

  const generatePDF = () => {
    const printWindow = window.open('', '_blank');
    const logoBase64 = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg=='; // Placeholder
    
    const mealsText = (meals) => {
      const m = [];
      if (meals?.breakfast) m.push('Kahvaltı');
      if (meals?.lunch) m.push('Öğle');
      if (meals?.dinner) m.push('Akşam');
    ...
    printWindow.document.write(`
      ...
      <html>
      <head>
        <title>${formData.name} - Tur Programı</title>
        <style>
          ...
          .tour-title { font-size: 28px; color: #ef4444; margin: 10px 0; }
          .tour-dates { font-size: 16px; color: #666; }
          .section { margin: 20px 0; }
          .section-title { font-size: 18px; color: #ef4444; border-bottom: 2px solid #eee; padding-bottom: 5px; margin-bottom: 15px; }
          .info-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; }
          .info-item { padding: 8px; background: #f9f9f9; border-radius: 5px; }
          .info-label { font-weight: bold; color: #666; font-size: 12px; }
          .info-value { font-size: 14px; margin-top: 3px; }
          .day-box { border: 1px solid #ddd; border-radius: 8px; margin-bottom: 15px; overflow: hidden; }
          .day-header { background: #ef4444; color: white; padding: 10px 15px; font-weight: bold; }
          .day-content { padding: 15px; }
          .activity { margin: 8px 0; padding: 8px; background: #f5f5f5; border-radius: 5px; }
          .activity-time { font-weight: bold; color: #ef4444; }
          .meals-hotel { display: flex; justify-content: space-between; margin-top: 10px; padding-top: 10px; border-top: 1px dashed #ddd; }
          .footer { margin-top: 30px; padding-top: 20px; border-top: 2px solid #eee; text-align: center; font-size: 12px; color: #666; }
          @media print { body { padding: 0; } }
        </style>
      </head>
      <body>
        <div class="header">
          <div style="font-size: 40px; color: #ef4444;">✈️ Paydos</div>
          <div style="font-size: 12px; color: #666;">"Just Travel"</div>
          <h1 class="tour-title">${formData.name}</h1>
          <div class="tour-dates">${formatDate(formData.startDate)} - ${formatDate(formData.endDate)} (${formData.duration} Gün)</div>
        </div>

        <div class="section">
          <h2 class="section-title">📋 Tur Bilgileri</h2>
          <div class="info-grid">
            <div class="info-item"><div class="info-label">Destinasyon</div><div class="info-value">${formData.destination}</div></div>
            <div class="info-item"><div class="info-label">Fiyat</div><div class="info-value">${formData.price} ${formData.currency}</div></div>
            <div class="info-item"><div class="info-label">Rehber</div><div class="info-value">${formData.guideName || '-'}</div></div>
            <div class="info-item"><div class="info-label">Rehber Tel</div><div class="info-value">${formData.guidePhone || '-'}</div></div>
          </div>
        </div>

        ${formData.departureFlightNo ? `
        <div class="section">
          <h2 class="section-title">✈️ Uçuş Bilgileri</h2>
          <div class="info-grid">
            <div class="info-item"><div class="info-label">Gidiş Uçuşu</div><div class="info-value">${formData.departureFlightNo} - ${formData.departureFlightTime || ''}</div></div>
            <div class="info-item"><div class="info-label">Gidiş Güzergah</div><div class="info-value">${formData.departureAirport || ''} → ${formData.arrivalAirport || ''}</div></div>
            <div class="info-item"><div class="info-label">Dönüş Uçuşu</div><div class="info-value">${formData.returnFlightNo || '-'} - ${formData.returnFlightTime || ''}</div></div>
            <div class="info-item"><div class="info-label">Dönüş Güzergah</div><div class="info-value">${formData.returnDepartureAirport || ''} → ${formData.returnArrivalAirport || ''}</div></div>
          </div>
        </div>
        ` : ''}
        ...
        <div class="section">
          <h2 class="section-title">✅ Dahil Olan Hizmetler</h2>
          <p>${formData.includedServices.replace(/\n/g, '<br>')}</p>
        </div>
        ` : ''}

        ${formData.excludedServices ? `
        <div class="section">
          <h2 class="section-title">❌ Dahil Olmayan Hizmetler</h2>
          <p>${formData.excludedServices.replace(/\n/g, '<br>')}</p>
        </div>
        ` : ''}

        <div class="section">
          <h2 class="section-title">📅 Günlük Program</h2>
          ${formData.dailyProgram.map(day => `
            <div class="day-box">
              <div class="day-header">${day.day}. Gün - ${formatDate(day.date)}</div>
              <div class="day-content">
                ${day.morningActivity ? `<div class="activity"><span class="activity-time">🌅 ${day.morningTime}</span> ${day.morningActivity}</div>` : ''}
                ${day.afternoonActivity ? `<div class="activity"><span class="activity-time">☀️ ${day.afternoonTime}</span> ${day.afternoonActivity}</div>` : ''}
                ${day.eveningActivity ? `<div class="activity"><span class="activity-time">🌙 ${day.eveningTime}</span> ${day.eveningActivity}</div>` : ''}
                <div class="meals-hotel">
                  <div><strong>🍽️ Öğünler:</strong> ${mealsText(day.meals)}</div>
                  <div><strong>🏨 Konaklama:</strong> ${day.hotel || '-'}</div>
                </div>
                ${day.notes ? `<div style="margin-top:10px;font-style:italic;color:#666;">📝 ${day.notes}</div>` : ''}
              </div>
            </div>
          `).join('')}
        </div>
        ...
        <div class="section">
          <h2 class="section-title">🆘 Acil Durum İletişim</h2>
          <p>${formData.emergencyContact}</p>
        </div>
        ` : ''}

        <div class="footer">
          <p><strong>Paydos Travel</strong> - "Just Travel"</p>
          <p>Bu program bilgilendirme amaçlıdır. Değişiklik hakkı saklıdır.</p>
        </div>
  ...
  const filteredCustomers = customers.filter(c => {
    if (!customerSearch) return true;
    const s = customerSearch.toLowerCase();
    return c.firstName?.toLowerCase().includes(s) || c.lastName?.toLowerCase().includes(s) || c.tcKimlik?.includes(customerSearch);
  });

  const tabs = [
    { id: 'general', label: 'Genel Bilgiler', icon: '📋' },
    { id: 'program', label: 'Günlük Program', icon: '📅' },
    { id: 'flights', label: 'Uçuş & Transfer', icon: '✈️' },
    { id: 'participants', label: `Katılımcılar (${formData.participants.length})`, icon: '👥' }
  ];
  ...
  if (view === 'list') {
    return (
      <div style={{ width: '100%', maxWidth: '100%', overflow: 'hidden' }}>
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2, 1fr)' : 'repeat(4, 1fr)', gap: '10px', marginBottom: '14px' }}>
          <StatCard value={tours.length} label="Toplam Tur" color="#10b981" />
          <StatCard value={tours.filter(t => t.status === 'Satışta').length} label="Satışta" color="#3b82f6" />
          <StatCard value={tours.filter(t => t.status === 'Devam Ediyor').length} label="Devam Eden" color="#8b5cf6" />
          <StatCard value={tours.reduce((sum, t) => sum + (t.participants?.length || 0), 0)} label="Toplam Katılımcı" color="#f59e0b" />
        </div>

        <div style={{ display: 'flex', gap: '8px', marginBottom: '12px', flexWrap: 'wrap' }}>
          <input type="text" placeholder="Tur ara..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} style={{ flex: 1, minWidth: '150px', padding: '10px 14px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', color: '#e8f1f8', fontSize: '14px', outline: 'none' }} />
          <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} style={{ padding: '10px 14px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', color: '#e8f1f8', fontSize: '14px', outline: 'none' }}>
            <option value="all">Tüm Durumlar</option>
            {statusOptions.map(s => <option key={s} value={s}>{s}</option>)}
          </select>
          <button onClick={() => { setEditingTour(null); setFormData(emptyTour); setActiveTab('general'); setView('edit'); }} style={{ padding: '10px 16px', background: 'linear-gradient(135deg, #10b981, #059669)', border: 'none', borderRadius: '8px', color: 'white', fontWeight: '600', cursor: 'pointer', fontSize: '14px' }}>+ Yeni Tur</button>
        </div>

        <div style={{ display: 'grid', gap: '12px' }}>
          {filteredTours.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '40px', color: '#94a3b8' }}>Henüz tur bulunmuyor</div>
          ) : (
            filteredTours.map(tour => (
              <div key={tour.id} style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '12px', padding: '16px', cursor: 'pointer' }} onClick={() => { setEditingTour(tour); setFormData(tour); setView('edit'); }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px' }}>
                  <div>
                    <h3 style={{ margin: 0, fontSize: '16px', color: '#e8f1f8' }}>🌍 {tour.name}</h3>
                    <p style={{ margin: '4px 0 0', fontSize: '13px', color: '#94a3b8' }}>{tour.destination}</p>
                  </div>
                  <span style={{ padding: '4px 10px', borderRadius: '12px', fontSize: '11px', fontWeight: '600', background: `${statusColors[tour.status]}20`, color: statusColors[tour.status] }}>{tour.status}</span>
                </div>
                <div style={{ display: 'flex', gap: '16px', fontSize: '12px', color: '#94a3b8', flexWrap: 'wrap' }}>
                  ...
                  <span>⏱️ {tour.duration} gün</span>
                  <span>👥 {tour.participants?.length || 0}/{tour.capacity}</span>
  ...
  return (
    <div style={{ width: '100%', maxWidth: '100%', overflow: 'hidden' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
        <button onClick={() => { setView('list'); setEditingTour(null); setFormData(emptyTour); }} style={{ padding: '8px 14px', background: 'rgba(255,255,255,0.1)', border: 'none', borderRadius: '8px', color: '#94a3b8', cursor: 'pointer', fontSize: '13px' }}>← Geri</button>
        <h2 style={{ margin: 0, fontSize: '18px', color: '#10b981' }}>{editingTour ? 'Tur Düzenle' : 'Yeni Tur'}</h2>
        <div style={{ display: 'flex', gap: '8px' }}>
          {editingTour && <button onClick={generatePDF} style={{ padding: '8px 14px', background: 'rgba(239,68,68,0.2)', border: '1px solid rgba(239,68,68,0.4)', borderRadius: '8px', color: '#ef4444', cursor: 'pointer', fontSize: '13px' }}>📄 PDF</button>}
          {editingTour && <button onClick={() => handleDelete(editingTour.id)} style={{ padding: '8px 14px', background: 'rgba(239,68,68,0.2)', border: '1px solid rgba(239,68,68,0.4)', borderRadius: '8px', color: '#ef4444', cursor: 'pointer', fontSize: '13px' }}>🗑️</button>}
        </div>
      </div>

      {saveMessage && <div style={{ background: 'rgba(16,185,129,0.2)', border: '1px solid rgba(16,185,129,0.4)', padding: '10px 14px', borderRadius: '8px', marginBottom: '12px', color: '#10b981', textAlign: 'center', fontWeight: '600' }}>{saveMessage}</div>}

      <div style={{ display: 'flex', gap: '6px', marginBottom: '16px', overflowX: 'auto', paddingBottom: '4px' }}>
        {tabs.map(tab => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{ padding: '10px 16px', background: activeTab === tab.id ? 'rgba(16,185,129,0.2)' : 'rgba(255,255,255,0.05)', border: activeTab === tab.id ? '1px solid rgba(16,185,129,0.4)' : '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', color: activeTab === tab.id ? '#10b981' : '#94a3b8', cursor: 'pointer', fontSize: '13px', whiteSpace: 'nowrap' }}>{tab.icon} {tab.label}</button>
        ))}
      </div>
      ...
      <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '12px', padding: '20px', marginBottom: '16px' }}>
        {activeTab === 'general' && (
          <div style={{ display: 'grid', gap: '14px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '14px' }}>
              <FormInput label="Tur Adı *" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} placeholder="örn: Yunanistan Adaları Turu" />
            ...
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : '1fr 1fr 1fr', gap: '14px' }}>
              <FormInput label="Fiyat" type="number" value={formData.price} onChange={(e) => setFormData({ ...formData, price: e.target.value })} />
              <div>
                <label style={{ display: 'block', fontSize: '12px', color: '#94a3b8', marginBottom: '6px' }}>Para Birimi</label>
                <select value={formData.currency} onChange={(e) => setFormData({ ...formData, currency: e.target.value })} style={{ width: '100%', padding: '10px 12px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '8px', color: '#e8f1f8', fontSize: '14px', outline: 'none' }}>
              ...
              <div>
                <label style={{ display: 'block', fontSize: '12px', color: '#94a3b8', marginBottom: '6px' }}>Durum</label>
                <select value={formData.status} onChange={(e) => setFormData({ ...formData, status: e.target.value })} style={{ width: '100%', padding: '10px 12px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '8px', color: '#e8f1f8', fontSize: '14px', outline: 'none' }}>
                  {statusOptions.map(s => <option key={s} value={s}>{s}</option>)}
                </select>
              </div>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '14px' }}>
              <FormInput label="Rehber Adı" value={formData.guideName} onChange={(e) => setFormData({ ...formData, guideName: e.target.value })} />
              <FormInput label="Rehber Telefon" value={formData.guidePhone} onChange={(e) => setFormData({ ...formData, guidePhone: e.target.value })} />
            </div>
            <div>
              <label style={{ display: 'block', fontSize: '12px', color: '#94a3b8', marginBottom: '6px' }}>Dahil Olan Hizmetler</label>
              <textarea value={formData.includedServices} onChange={(e) => setFormData({ ...formData, includedServices: e.target.value })} rows={3} placeholder="Her satıra bir hizmet..." style={{ width: '100%', padding: '10px 12px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '8px', color: '#e8f1f8', fontSize: '14px', outline: 'none', resize: 'vertical' }} />
            </div>
            <div>
              <label style={{ display: 'block', fontSize: '12px', color: '#94a3b8', marginBottom: '6px' }}>Dahil Olmayan Hizmetler</label>
              <textarea value={formData.excludedServices} onChange={(e) => setFormData({ ...formData, excludedServices: e.target.value })} rows={3} placeholder="Her satıra bir hizmet..." style={{ width: '100%', padding: '10px 12px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '8px', color: '#e8f1f8', fontSize: '14px', outline: 'none', resize: 'vertical' }} />
            </div>
          </div>
        )}
        ...
        {activeTab === 'program' && (
          <div>
            {formData.dailyProgram.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '30px', color: '#94a3b8' }}>
                <p>Önce Genel Bilgiler sekmesinden tarihleri girin.</p>
              </div>
            ) : (
              formData.dailyProgram.map((day, idx) => (
                <div key={idx} style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '10px', padding: '16px', marginBottom: '12px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                    <h4 style={{ margin: 0, color: '#10b981' }}>📅 {day.day}. Gün - {formatDate(day.date)}</h4>
                  </div>
                  <div style={{ display: 'grid', gap: '10px' }}>
                    <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '80px 1fr', gap: '10px', alignItems: 'center' }}>
                      <input type="time" value={day.morningTime} onChange={(e) => updateDayProgram(idx, 'morningTime', e.target.value)} style={{ padding: '8px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '6px', color: '#e8f1f8', fontSize: '13px' }} />
                      <input type="text" value={day.morningActivity} onChange={(e) => updateDayProgram(idx, 'morningActivity', e.target.value)} placeholder="🌅 Sabah aktivitesi" style={{ padding: '10px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '6px', color: '#e8f1f8', fontSize: '13px' }} />
                    </div>
                    <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '80px 1fr', gap: '10px', alignItems: 'center' }}>
                      <input type="time" value={day.afternoonTime} onChange={(e) => updateDayProgram(idx, 'afternoonTime', e.target.value)} style={{ padding: '8px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '6px', color: '#e8f1f8', fontSize: '13px' }} />
                      <input type="text" value={day.afternoonActivity} onChange={(e) => updateDayProgram(idx, 'afternoonActivity', e.target.value)} placeholder="☀️ Öğle aktivitesi" style={{ padding: '10px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '6px', color: '#e8f1f8', fontSize: '13px' }} />
                    </div>
                    <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '80px 1fr', gap: '10px', alignItems: 'center' }}>
                      <input type="time" value={day.eveningTime} onChange={(e) => updateDayProgram(idx, 'eveningTime', e.target.value)} style={{ padding: '8px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '6px', color: '#e8f1f8', fontSize: '13px' }} />
                      <input type="text" value={day.eveningActivity} onChange={(e) => updateDayProgram(idx, 'eveningActivity', e.target.value)} placeholder="🌙 Akşam aktivitesi" style={{ padding: '10px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '6px', color: '#e8f1f8', fontSize: '13px' }} />
                    </div>
                    <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '10px' }}>
                      <input type="text" value={day.hotel} onChange={(e) => updateDayProgram(idx, 'hotel', e.target.value)} placeholder="🏨 Otel adı" style={{ padding: '10px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '6px', color: '#e8f1f8', fontSize: '13px' }} />
                      <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                        <label style={{ display: 'flex', alignItems: 'center', gap: '4px', color: '#94a3b8', fontSize: '12px', cursor: 'pointer' }}>
                          <input type="checkbox" checked={day.meals?.breakfast || false} onChange={(e) => updateDayProgram(idx, 'meals.breakfast', e.target.checked)} /> Kahvaltı
                        </label>
                        <label style={{ display: 'flex', alignItems: 'center', gap: '4px', color: '#94a3b8', fontSize: '12px', cursor: 'pointer' }}>
                          <input type="checkbox" checked={day.meals?.lunch || false} onChange={(e) => updateDayProgram(idx, 'meals.lunch', e.target.checked)} /> Öğle
                        </label>
                        <label style={{ display: 'flex', alignItems: 'center', gap: '4px', color: '#94a3b8', fontSize: '12px', cursor: 'pointer' }}>
                          <input type="checkbox" checked={day.meals?.dinner || false} onChange={(e) => updateDayProgram(idx, 'meals.dinner', e.target.checked)} /> Akşam
                        </label>
                      </div>
                    </div>
                    <input type="text" value={day.notes} onChange={(e) => updateDayProgram(idx, 'notes', e.target.value)} placeholder="📝 Gün notu (opsiyonel)" style={{ padding: '10px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '6px', color: '#e8f1f8', fontSize: '13px' }} />
                  </div>
                </div>
              ))
            )}
        ...
        {activeTab === 'flights' && (
          <div style={{ display: 'grid', gap: '20px' }}>
            <div>
              <h4 style={{ margin: '0 0 12px', color: '#3b82f6' }}>✈️ Gidiş Uçuşu</h4>
              <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr 1fr 1fr', gap: '12px' }}>
                <FormInput label="Uçuş No" value={formData.departureFlightNo} onChange={(e) => setFormData({ ...formData, departureFlightNo: e.target.value })} placeholder="TK1234" />
                <FormInput label="Saat" type="time" value={formData.departureFlightTime} onChange={(e) => setFormData({ ...formData, departureFlightTime: e.target.value })} />
                <FormInput label="Kalkış" value={formData.departureAirport} onChange={(e) => setFormData({ ...formData, departureAirport: e.target.value })} placeholder="İstanbul (IST)" />
                <FormInput label="Varış" value={formData.arrivalAirport} onChange={(e) => setFormData({ ...formData, arrivalAirport: e.target.value })} placeholder="Atina (ATH)" />
              </div>
            </div>
            <div>
              <h4 style={{ margin: '0 0 12px', color: '#10b981' }}>✈️ Dönüş Uçuşu</h4>
              <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr 1fr 1fr', gap: '12px' }}>
                <FormInput label="Uçuş No" value={formData.returnFlightNo} onChange={(e) => setFormData({ ...formData, returnFlightNo: e.target.value })} placeholder="TK1235" />
                <FormInput label="Saat" type="time" value={formData.returnFlightTime} onChange={(e) => setFormData({ ...formData, returnFlightTime: e.target.value })} />
                <FormInput label="Kalkış" value={formData.returnDepartureAirport} onChange={(e) => setFormData({ ...formData, returnDepartureAirport: e.target.value })} placeholder="Atina (ATH)" />
                <FormInput label="Varış" value={formData.returnArrivalAirport} onChange={(e) => setFormData({ ...formData, returnArrivalAirport: e.target.value })} placeholder="İstanbul (IST)" />
              </div>
            </div>
            <div>
              <label style={{ display: 'block', fontSize: '12px', color: '#94a3b8', marginBottom: '6px' }}>🚐 Transfer Notları</label>
              <textarea value={formData.transferNotes} onChange={(e) => setFormData({ ...formData, transferNotes: e.target.value })} rows={3} placeholder="Transfer detayları..." style={{ width: '100%', padding: '10px 12px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '8px', color: '#e8f1f8', fontSize: '14px', outline: 'none', resize: 'vertical' }} />
            </div>
            <FormInput label="🆘 Acil Durum İletişim" value={formData.emergencyContact} onChange={(e) => setFormData({ ...formData, emergencyContact: e.target.value })} placeholder="İsim, telefon..." />
          </div>
        )}
        ...
        {activeTab === 'participants' && (
          <div>
            <div style={{ marginBottom: '16px' }}>
              <input type="text" placeholder="Müşteri ara (Ad, TC)..." value={customerSearch} onChange={(e) => setCustomerSearch(e.target.value)} style={{ width: '100%', padding: '10px 14px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', color: '#e8f1f8', fontSize: '14px', outline: 'none', marginBottom: '10px' }} />
              {customerSearch && (
                <div style={{ maxHeight: '200px', overflowY: 'auto', background: 'rgba(0,0,0,0.3)', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.1)' }}>
                  {filteredCustomers.slice(0, 10).map(c => (
                    <div key={c.id} onClick={() => { addParticipant(c); setCustomerSearch(''); }} style={{ padding: '10px 14px', borderBottom: '1px solid rgba(255,255,255,0.05)', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div>
                        <div style={{ color: '#e8f1f8', fontSize: '14px' }}>{c.firstName} {c.lastName}</div>
                        <div style={{ color: '#94a3b8', fontSize: '11px' }}>{c.tcKimlik} • {c.phone}</div>
                      </div>
                      <span style={{ color: '#10b981' }}>+ Ekle</span>
                    </div>
                  ))}
                </div>
              )}
            ...
            {formData.participants.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '30px', color: '#94a3b8' }}>Henüz katılımcı eklenmedi</div>
            ) : (
              <div style={{ display: 'grid', gap: '10px' }}>
                {formData.participants.map((p, idx) => (
                  <div key={p.customerId} style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '10px', padding: '14px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px' }}>
                      <div>
                        <div style={{ color: '#e8f1f8', fontWeight: '600' }}>{idx + 1}. {p.name}</div>
                        <div style={{ color: '#94a3b8', fontSize: '12px' }}>{p.tcKimlik} • {p.phone}</div>
                      </div>
                      <button onClick={() => removeParticipant(p.customerId)} style={{ background: 'rgba(239,68,68,0.2)', border: 'none', borderRadius: '6px', color: '#ef4444', padding: '4px 8px', cursor: 'pointer', fontSize: '11px' }}>Çıkar</button>
                    </div>
                    <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr 1fr', gap: '10px' }}>
                      <div>
                        <label style={{ fontSize: '11px', color: '#94a3b8' }}>Ödeme Durumu</label>
                        <select value={p.paymentStatus} onChange={(e) => updateParticipant(p.customerId, 'paymentStatus', e.target.value)} style={{ width: '100%', padding: '8px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '6px', color: '#e8f1f8', fontSize: '13px' }}>
                          <option value="Ödenmedi">Ödenmedi</option>
                          <option value="Kısmi">Kısmi Ödeme</option>
                          <option value="Ödendi">Ödendi</option>
                        </select>
                      </div>
                      <FormInput label="Ödenen Tutar" type="number" value={p.paidAmount} onChange={(e) => updateParticipant(p.customerId, 'paidAmount', e.target.value)} />
                      <FormInput label="Not" value={p.notes} onChange={(e) => updateParticipant(p.customerId, 'notes', e.target.value)} />
                    </div>
                  </div>
                ))}
              </div>
      ...
      <button onClick={handleSave} style={{ width: '100%', padding: '14px', background: 'linear-gradient(135deg, #10b981, #059669)', border: 'none', borderRadius: '10px', color: 'white', fontWeight: '700', cursor: 'pointer', fontSize: '15px' }}>💾 Kaydet</button>
    </div>
  );
}

function HotelModule({ hotelReservations, addHotel, updateHotel, deleteHotel, customers, isMobile }) {
  const [view, setView] = useState('list'); // list, hotels, editHotel, reservation, editReservation
  const [searchTerm, setSearchTerm] = useState('');
  const [filterStatus, setFilterStatus] = useState('all');
  const [saveMessage, setSaveMessage] = useState('');
  const [hotels, setHotels] = useState(() => {
    try { return JSON.parse(localStorage.getItem('paydos_hotels') || '[]'); } catch { return []; }
  });
  ...
  const emptyHotel = {
    name: '', city: '', country: '', address: '', stars: '5', phone: '', email: '', website: '',
    contactPerson: '', commissionType: 'percent', commissionValue: 10, notes: '',
    roomTypes: ['Standart Oda', 'Deluxe Oda', 'Suite'],
    prices: []
  };
  const [hotelForm, setHotelForm] = useState(emptyHotel);

  const emptyReservation = {
    hotelId: '', hotelName: '', customerId: '', customerName: '', customerPhone: '',
    roomType: '', boardType: 'BB', checkIn: '', checkOut: '', nights: 1,
    adults: 2, children: 0, childAges: '', pricePerNight: '', totalPrice: '',
    currency: '€', commissionAmount: '', status: 'Beklemede', paymentStatus: 'Ödenmedi',
    paidAmount: 0, confirmationNo: '', notes: ''
  };
  ...
  const statusColors = { 'Beklemede': '#f59e0b', 'Onaylı': '#10b981', 'İptal': '#ef4444', 'Tamamlandı': '#3b82f6' };
  const paymentColors = { 'Ödenmedi': '#ef4444', 'Kısmi': '#f59e0b', 'Ödendi': '#10b981' };

  // localStorage'a otele kaydet
  useEffect(() => {
    localStorage.setItem('paydos_hotels', JSON.stringify(hotels));
  }, [hotels]);

  const filteredReservations = hotelReservations.filter(r => {
    if (filterStatus !== 'all' && r.status !== filterStatus) return false;
    if (!searchTerm) return true;
    return r.customerName?.toLowerCase().includes(searchTerm.toLowerCase()) || r.hotelName?.toLowerCase().includes(searchTerm.toLowerCase());
  });
  ...
  const calculateCommission = (total, hotel) => {
    if (!hotel || !total) return 0;
    if (hotel.commissionType === 'percent') {
      return (parseFloat(total) * parseFloat(hotel.commissionValue) / 100).toFixed(2);
    }
    return parseFloat(hotel.commissionValue) || 0;
  };

  const handleReservationDateChange = (field, value) => {
    ...
    if (newData.checkIn && newData.checkOut) {
      ...
      if (newData.pricePerNight) {
        newData.totalPrice = (parseFloat(newData.pricePerNight) * newData.nights).toFixed(2);
  ...
  const handlePriceChange = (value) => {
    const newData = { ...reservationForm, pricePerNight: value };
    if (newData.nights && value) {
      newData.totalPrice = (parseFloat(value) * newData.nights).toFixed(2);
      const hotel = hotels.find(h => h.id === newData.hotelId);
      newData.commissionAmount = calculateCommission(newData.totalPrice, hotel);
    }
    setReservationForm(newData);
  ...
  const saveHotel = () => {
    if (!hotelForm.name) { alert('Otel adı zorunludur!'); return; }
    if (editingHotel) {
      setHotels(hotels.map(h => h.id === editingHotel.id ? { ...editingHotel, ...hotelForm } : h));
    } else {
      setHotels([...hotels, { id: Date.now(), ...hotelForm }]);
    }
    setSaveMessage('Otel kaydedildi ✓');
    setTimeout(() => { setSaveMessage(''); setView('hotels'); setEditingHotel(null); setHotelForm(emptyHotel); }, 1500);
  };

  const deleteHotelItem = (id) => {
  ...
  const saveReservation = async () => {
    if (!reservationForm.hotelId) { alert('Otel seçiniz!'); return; }
    if (!reservationForm.customerId) { alert('Müşteri seçiniz!'); return; }
    if (!reservationForm.checkIn || !reservationForm.checkOut) { alert('Tarihler zorunludur!'); return; }

    if (editingReservation) {
      await updateHotel({ ...editingReservation, ...reservationForm });
    } else {
      await addHotel({ ...reservationForm, createdAt: new Date().toISOString() });
    }
    setSaveMessage('Rezervasyon kaydedildi ✓');
    setTimeout(() => { setSaveMessage(''); setView('list'); setEditingReservation(null); setReservationForm(emptyReservation); }, 1500);
  };
  ...
  const totalCommission = hotelReservations.reduce((sum, r) => sum + (parseFloat(r.commissionAmount) || 0), 0);
  ...
  if (view === 'hotels') {
    return (
      <div style={{ width: '100%', maxWidth: '100%', overflow: 'hidden' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          <button onClick={() => setView('list')} style={{ padding: '8px 14px', background: 'rgba(255,255,255,0.1)', border: 'none', borderRadius: '8px', color: '#94a3b8', cursor: 'pointer' }}>← Geri</button>
          <h2 style={{ margin: 0, fontSize: '18px', color: '#8b5cf6' }}>🏨 Otel Yönetimi</h2>
          <button onClick={() => { setEditingHotel(null); setHotelForm(emptyHotel); setView('editHotel'); }} style={{ padding: '8px 14px', background: 'linear-gradient(135deg, #8b5cf6, #7c3aed)', border: 'none', borderRadius: '8px', color: 'white', cursor: 'pointer' }}>+ Yeni Otel</button>
        </div>

        {hotels.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '40px', color: '#94a3b8' }}>Henüz otel eklenmedi</div>
        ) : (
          <div style={{ display: 'grid', gap: '12px' }}>
            {hotels.map(hotel => (
              <div key={hotel.id} style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '12px', padding: '16px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div>
                    <h3 style={{ margin: 0, color: '#e8f1f8' }}>{hotel.name} {'⭐'.repeat(parseInt(hotel.stars) || 0)}</h3>
                    <p style={{ margin: '4px 0', color: '#94a3b8', fontSize: '13px' }}>📍 {hotel.city}, {hotel.country}</p>
                    <p style={{ margin: '4px 0', color: '#94a3b8', fontSize: '12px' }}>💰 Komisyon: %{hotel.commissionValue}</p>
                  </div>
                  <div style={{ display: 'flex', gap: '8px' }}>
                    <button onClick={() => { setEditingHotel(hotel); setHotelForm(hotel); setView('editHotel'); }} style={{ padding: '6px 12px', background: 'rgba(59,130,246,0.2)', border: 'none', borderRadius: '6px', color: '#3b82f6', cursor: 'pointer', fontSize: '12px' }}>Düzenle</button>
                    <button onClick={() => deleteHotelItem(hotel.id)} style={{ padding: '6px 12px', background: 'rgba(239,68,68,0.2)', border: 'none', borderRadius: '6px', color: '#ef4444', cursor: 'pointer', fontSize: '12px' }}>Sil</button>
                  </div>
                </div>
              </div>
            ))}
  ...
  if (view === 'editHotel') {
    return (
      <div style={{ width: '100%', maxWidth: '100%', overflow: 'hidden' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          <button onClick={() => { setView('hotels'); setEditingHotel(null); setHotelForm(emptyHotel); }} style={{ padding: '8px 14px', background: 'rgba(255,255,255,0.1)', border: 'none', borderRadius: '8px', color: '#94a3b8', cursor: 'pointer' }}>← Geri</button>
          <h2 style={{ margin: 0, fontSize: '18px', color: '#8b5cf6' }}>{editingHotel ? 'Otel Düzenle' : 'Yeni Otel'}</h2>
          <div></div>
        </div>
          ...
          <div style={{ display: 'grid', gap: '14px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr 1fr', gap: '14px' }}>
              ...
              <div>
                ...
                <select value={hotelForm.stars} onChange={(e) => setHotelForm({ ...hotelForm, stars: e.target.value })} style={{ width: '100%', padding: '10px 12px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '8px', color: '#e8f1f8' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '14px' }}>
              ...
              <FormInput label={hotelForm.commissionType === 'percent' ? 'Komisyon (%)' : 'Komisyon Tutarı'} type="number" value={hotelForm.commissionValue} onChange={(e) => setHotelForm({ ...hotelForm, commissionValue: e.target.value })} />
            </div>
  ...
  if (view === 'editReservation') {
    return (
      <div style={{ width: '100%', maxWidth: '100%', overflow: 'hidden' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          <button onClick={() => { setView('list'); setEditingReservation(null); setReservationForm(emptyReservation); }} style={{ padding: '8px 14px', background: 'rgba(255,255,255,0.1)', border: 'none', borderRadius: '8px', color: '#94a3b8', cursor: 'pointer' }}>← Geri</button>
          <h2 style={{ margin: 0, fontSize: '18px', color: '#8b5cf6' }}>{editingReservation ? 'Rezervasyon Düzenle' : 'Yeni Rezervasyon'}</h2>
          <div></div>
        </div>
        ...
        <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '12px', padding: '20px', marginBottom: '16px' }}>
          <div style={{ display: 'grid', gap: '16px' }}>
            {/* Otel Seçimi */}
            <div>
              <label style={{ display: 'block', fontSize: '12px', color: '#94a3b8', marginBottom: '8px' }}>🏨 Otel Seç *</label>
              {reservationForm.hotelId ? (
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px', background: 'rgba(139,92,246,0.1)', border: '1px solid rgba(139,92,246,0.3)', borderRadius: '8px' }}>
                  <span style={{ color: '#e8f1f8' }}>{reservationForm.hotelName}</span>
                  <button onClick={() => setReservationForm({ ...reservationForm, hotelId: '', hotelName: '' })} style={{ background: 'none', border: 'none', color: '#ef4444', cursor: 'pointer' }}>Değiştir</button>
                </div>
              ) : (
                <div style={{ display: 'grid', gap: '8px', maxHeight: '200px', overflowY: 'auto' }}>
                  {hotels.length === 0 ? (
                    <div style={{ padding: '20px', textAlign: 'center', color: '#94a3b8' }}>
                      <p>Önce otel ekleyin</p>
                      <button onClick={() => setView('hotels')} style={{ marginTop: '8px', padding: '8px 16px', background: 'rgba(139,92,246,0.2)', border: '1px solid rgba(139,92,246,0.4)', borderRadius: '6px', color: '#8b5cf6', cursor: 'pointer' }}>Otel Ekle</button>
                    </div>
                  ) : (
                    hotels.map(h => (
                      <div key={h.id} onClick={() => selectHotelForReservation(h)} style={{ padding: '10px', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '8px', cursor: 'pointer' }}>
                        <div style={{ color: '#e8f1f8' }}>{h.name} {'⭐'.repeat(parseInt(h.stars) || 0)}</div>
                        <div style={{ color: '#94a3b8', fontSize: '12px' }}>{h.city}, {h.country}</div>
                      </div>
                    ))
                  )}
                </div>
            ...
            {/* Müşteri Seçimi */}
            <div>
              <label style={{ display: 'block', fontSize: '12px', color: '#94a3b8', marginBottom: '8px' }}>👤 Müşteri Seç *</label>
              {reservationForm.customerId ? (
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px', background: 'rgba(16,185,129,0.1)', border: '1px solid rgba(16,185,129,0.3)', borderRadius: '8px' }}>
                  <div>
                    <span style={{ color: '#e8f1f8' }}>{reservationForm.customerName}</span>
                    <span style={{ color: '#94a3b8', fontSize: '12px', marginLeft: '8px' }}>{reservationForm.customerPhone}</span>
                  </div>
                  <button onClick={() => setReservationForm({ ...reservationForm, customerId: '', customerName: '', customerPhone: '' })} style={{ background: 'none', border: 'none', color: '#ef4444', cursor: 'pointer' }}>Değiştir</button>
                </div>
              ) : (
                <div>
                  <input type="text" placeholder="Müşteri ara..." value={customerSearch} onChange={(e) => setCustomerSearch(e.target.value)} style={{ width: '100%', padding: '10px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '8px', color: '#e8f1f8', marginBottom: '8px' }} />
                  {customerSearch && (
                    <div style={{ maxHeight: '150px', overflowY: 'auto' }}>
                      {filteredCustomers.slice(0, 5).map(c => (
                        <div key={c.id} onClick={() => selectCustomerForReservation(c)} style={{ padding: '8px', background: 'rgba(255,255,255,0.03)', borderRadius: '6px', marginBottom: '4px', cursor: 'pointer' }}>
                          <div style={{ color: '#e8f1f8', fontSize: '13px' }}>{c.firstName} {c.lastName}</div>
                          <div style={{ color: '#94a3b8', fontSize: '11px' }}>{c.phone}</div>
                        </div>
                      ))}
                    </div>
                  )}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '14px' }}>
              <FormInput label="Yetişkin" type="number" value={reservationForm.adults} onChange={(e) => setReservationForm({ ...reservationForm, adults: parseInt(e.target.value) || 1 })} />
              <FormInput label="Çocuk" type="number" value={reservationForm.children} onChange={(e) => setReservationForm({ ...reservationForm, children: parseInt(e.target.value) || 0 })} />
              {reservationForm.children > 0 && <FormInput label="Çocuk Yaşları" value={reservationForm.childAges} onChange={(e) => setReservationForm({ ...reservationForm, childAges: e.target.value })} placeholder="6, 12" />}
            </div>

            {/* Fiyatlandırma */}
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : '1fr 1fr 1fr 1fr', gap: '14px' }}>
              <FormInput label="Gecelik Fiyat" type="number" value={reservationForm.pricePerNight} onChange={(e) => handlePriceChange(e.target.value)} />
              <FormInput label="Toplam Tutar" type="number" value={reservationForm.totalPrice} readOnly />
              <div>
                <label style={{ display: 'block', fontSize: '12px', color: '#94a3b8', marginBottom: '6px' }}>Para Birimi</label>
                <select value={reservationForm.currency} onChange={(e) => setReservationForm({ ...reservationForm, currency: e.target.value })} style={{ width: '100%', padding: '10px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '8px', color: '#e8f1f8' }}>
                  <option value="€">€</option>
                  <option value="$">$</option>
                  <option value="₺">₺</option>
                </select>
              </div>
              <FormInput label="Komisyon" value={reservationForm.commissionAmount} readOnly />
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr 1fr', gap: '14px' }}>
              <div>
                <label style={{ display: 'block', fontSize: '12px', color: '#94a3b8', marginBottom: '6px' }}>Durum</label>
                <select value={reservationForm.status} onChange={(e) => setReservationForm({ ...reservationForm, status: e.target.value })} style={{ width: '100%', padding: '10px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '8px', color: '#e8f1f8' }}>
              ...
              <div>
                <label style={{ display: 'block', fontSize: '12px', color: '#94a3b8', marginBottom: '6px' }}>Ödeme Durumu</label>
                <select value={reservationForm.paymentStatus} onChange={(e) => setReservationForm({ ...reservationForm, paymentStatus: e.target.value })} style={{ width: '100%', padding: '10px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '8px', color: '#e8f1f8' }}>
                  <option value="Ödenmedi">Ödenmedi</option>
                  <option value="Kısmi">Kısmi Ödeme</option>
                  <option value="Ödendi">Ödendi</option>
                </select>
              </div>
              <FormInput label="Konfirmasyon No" value={reservationForm.confirmationNo} onChange={(e) => setReservationForm({ ...reservationForm, confirmationNo: e.target.value })} />
            </div>

            <div>
              <label style={{ display: 'block', fontSize: '12px', color: '#94a3b8', marginBottom: '6px' }}>Notlar</label>
              <textarea value={reservationForm.notes} onChange={(e) => setReservationForm({ ...reservationForm, notes: e.target.value })} rows={2} style={{ width: '100%', padding: '10px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '8px', color: '#e8f1f8', resize: 'vertical' }} />
            </div>
          </div>
        </div>

        <button onClick={saveReservation} style={{ width: '100%', padding: '14px', background: 'linear-gradient(135deg, #8b5cf6, #7c3aed)', border: 'none', borderRadius: '10px', color: 'white', fontWeight: '700', cursor: 'pointer', fontSize: '15px' }}>💾 Kaydet</button>
      </div>
    );
  }
  ...
  return (
    <div style={{ width: '100%', maxWidth: '100%', overflow: 'hidden' }}>
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2, 1fr)' : 'repeat(4, 1fr)', gap: '10px', marginBottom: '14px' }}>
        <StatCard value={hotelReservations.length} label="Toplam Rezervasyon" color="#8b5cf6" />
        <StatCard value={hotelReservations.filter(r => r.status === 'Onaylı').length} label="Onaylı" color="#10b981" />
        <StatCard value={hotelReservations.filter(r => r.status === 'Beklemede').length} label="Beklemede" color="#f59e0b" />
        <StatCard value={`${totalCommission.toFixed(0)}€`} label="Toplam Komisyon" color="#3b82f6" />
      </div>

      <div style={{ display: 'flex', gap: '8px', marginBottom: '12px', flexWrap: 'wrap' }}>
        <input type="text" placeholder="Rezervasyon ara..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} style={{ flex: 1, minWidth: '150px', padding: '10px 14px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', color: '#e8f1f8', fontSize: '14px', outline: 'none' }} />
        <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} style={{ padding: '10px 14px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', color: '#e8f1f8', fontSize: '14px' }}>
          <option value="all">Tüm Durumlar</option>
          <option value="Beklemede">Beklemede</option>
          <option value="Onaylı">Onaylı</option>
          <option value="Tamamlandı">Tamamlandı</option>
        ...
        <button onClick={() => setView('hotels')} style={{ padding: '10px 14px', background: 'rgba(139,92,246,0.2)', border: '1px solid rgba(139,92,246,0.4)', borderRadius: '8px', color: '#8b5cf6', cursor: 'pointer', fontSize: '13px' }}>🏨 Oteller</button>
        <button onClick={() => { setEditingReservation(null); setReservationForm(emptyReservation); setView('editReservation'); }} style={{ padding: '10px 16px', background: 'linear-gradient(135deg, #8b5cf6, #7c3aed)', border: 'none', borderRadius: '8px', color: 'white', fontWeight: '600', cursor: 'pointer', fontSize: '14px' }}>+ Yeni Rezervasyon</button>
      </div>

      <div style={{ display: 'grid', gap: '12px' }}>
        {filteredReservations.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '40px', color: '#94a3b8' }}>Henüz rezervasyon bulunmuyor</div>
        ) : (
          filteredReservations.map(res => (
            <div key={res.id} onClick={() => { setEditingReservation(res); setReservationForm(res); setView('editReservation'); }} style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '12px', padding: '16px', cursor: 'pointer' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px' }}>
                <div>
                  <h3 style={{ margin: 0, fontSize: '15px', color: '#e8f1f8' }}>👤 {res.customerName}</h3>
                  <p style={{ margin: '4px 0 0', fontSize: '13px', color: '#94a3b8' }}>🏨 {res.hotelName}</p>
                </div>
                <div style={{ display: 'flex', gap: '6px' }}>
                  <span style={{ padding: '4px 8px', borderRadius: '8px', fontSize: '10px', fontWeight: '600', background: `${statusColors[res.status]}20`, color: statusColors[res.status] }}>{res.status}</span>
                  <span style={{ padding: '4px 8px', borderRadius: '8px', fontSize: '10px', fontWeight: '600', background: `${paymentColors[res.paymentStatus]}20`, color: paymentColors[res.paymentStatus] }}>{res.paymentStatus}</span>
                </div>
              </div>
              <div style={{ display: 'flex', gap: '16px', fontSize: '12px', color: '#94a3b8', flexWrap: 'wrap' }}>
                <span>📅 {formatDate(res.checkIn)} - {formatDate(res.checkOut)}</span>
                <span>🌙 {res.nights} gece</span>
                <span>🍽️ {boardTypes.find(b => b.id === res.boardType)?.label || res.boardType}</span>
                <span>💰 {res.totalPrice} {res.currency}</span>
                <span style={{ color: '#10b981' }}>📊 +{res.commissionAmount} {res.currency}</span>
              </div>
            </div>
          ))
        )}
