...
function CustomerModule({ customers, setCustomers, isMobile, musteriEtiketleri }) {
  ...
  const importFromExcel = (e) => {
    ...
    reader.onload = (evt) => {
      ...
      const parseDate = (d) => { if (!d) return ''; if (typeof d === 'string' && d.includes('.')) { const [day, month, year] = d.split('.'); return `${year}-${month.padStart(2, '0')}-${day.padStart(2, '0')}`; } return d; };
  ...
  return (
    <div style={{ width: '100%', maxWidth: '100%', overflow: 'hidden' }}>
      ...
      {showModal && (
        <div onClick={handleClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(5px)', display: 'flex', alignItems: isMobile ? 'flex-end' : 'center', justifyContent: 'center', zIndex: 300, padding: isMobile ? '0' : '16px' }}>
          <div onClick={e => e.stopPropagation()} style={{ background: 'linear-gradient(180deg, #0f2744 0%, #0c1929 100%)', borderRadius: isMobile ? '14px 14px 0 0' : '12px', width: '100%', maxWidth: '550px', maxHeight: isMobile ? '88vh' : '82vh', overflow: 'hidden', border: '1px solid rgba(255,255,255,0.1)', display: 'flex', flexDirection: 'column' }}>
